...
import { pingOpenSearch, searchProductsOpenSearch } from './search.js';
...
app.get('/readyz', async (_req, res) => {
  ...
  const opensearch = await pingOpenSearch();
  const ok = db.ok === true;
  res.status(ok ? 200 : 503).json({ ok, db, cache, opensearch });
});
...
app.get('/status', async (_req, res) => {
  const status = {
    ...
    opensearch: await pingOpenSearch()
  };
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.send(formPage({ type: 'login', message: `Bem-vindo, ${customer.name}! Login validado com RDS MySQL.` }));
  } catch (err) {
...
async function searchProducts(query) {
  ...
  if (cached) return { products: cached, source: 'valkey-cache' };

  try {
    const opensearchProducts = await searchProductsOpenSearch(query);
    if (opensearchProducts) {
      await setJson(cacheKey, opensearchProducts);
      return { products: opensearchProducts, source: 'opensearch' };
    }
  } catch (err) {
    console.warn('[search] OpenSearch falhou, usando fallback MySQL:', err.message);
  }
